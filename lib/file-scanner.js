...
FileScanner.prototype = {
            ...
            if (err) {
                ...
                files.forEach(function (filename) {
                    ...
                    if (lstat.isFile() && self.isPatchFile(filename)) {
                        let version;
                        let action;
                        if(self.patchFileTemplateMode === "action-version"){
                            let version = self.getPatchFileVersion(filename);
                            let action = self.getPatchFileAction(filename);    
                        }else if(self.patchFileTemplateMode === "source-target"){
                            //TODO
                        }
    ...
    validatePatchFileTemplate: function(){
        ...
        if((found.action || found.version) && (found.source || found.target)){
            ...
            this.patchFileTemplateMode = "action-version";
            return q("action-version");
        }else if(found.source && found.target){
            this.patchFileTemplateMode = "source-target";
