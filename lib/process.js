...
let PgPatchProcess = function (config) {
    ...
    this.config.sourceVersion = config.sourceVersion === undefined ? null : config.sourceVersion;

    this.dryRun = dryRun[config.dryRun] || null;

    if(this.dryRun){ //force single transaction mode
        this.transactionMode = transactionMode.SINGLE;
    }else{
        this.transactionMode = transactionMode[config.transactionMode] || transactionMode.PER_VERSION_STEP;
    }
...
PgPatchProcess.prototype = {
    run: function() {
        ...
        return this.db.connect()
            ...
            .then(() => { //determine patch process information
                //sourceVersion
                ...
                if (cache.sourceVersion < 0) {
                    return Promise.reject(`Invalid sourceVersion: ${cache.sourceVersion}`);
                } else if (cache.sourceVersion > cache.maxPatchVersionFound) {
                ...
                if (cache.targetVersion < 0) {
                    return Promise.reject(`Invalid targetVersion: ${cache.targetVersion}`);
                } else if (cache.targetVersion > cache.maxPatchVersionFound) {
                    return Promise.reject(`Target patch version not found: ${config.targetVersion}; (Max found was: ${cache.maxPatchVersionFound})`);
                }
            })
    ...
    patch: function() {
        ...
        return q.all(versionReadPromises).then(transactionsData => {
            ...
            if(this.transactionMode === transactionMode.PER_VERSION_STEP) {
                ...
                return self.db.patchQuery(sqlToRun.join(';\n')).then(() => {
                    if (action === actions.UPDATE) {
                        return self.db.patchQuery(`update ${this.db.getDBPatchTableName()} set current_version = ${this.cache.targetVersion};`).then(() => {
                            this.console.success(`DB updated to version ${this.cache.targetVersion}`);
                        });
                    } else {
                        return self.db.patchQuery(`update ${this.db.getDBPatchTableName()} set current_version = ${this.cache.targetVersion - 1};`).then(() => {
