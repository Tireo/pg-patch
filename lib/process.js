            ...
            if(this.transactionMode === transactionMode.PER_VERSION_STEP) {
                ...
                transactionsData.forEach(transactionData => {
                    patchChainPromise = patchChainPromise.then(() => {
                        ...
                        if (action === actions.UPDATE) {
                            return this.db.updatePatchHistory(transactionData.version-1, transactionData.version).then(() => {
                                this.console.success(`DB updated to version ${transactionData.version}`);
                            });
                        } else {
                            return self.db.updatePatchHistory(transactionData.version, transactionData.version-1).then(() => {
                    ...
                    if(this.dryRun !== dryRun.TEST_SQL){
                        this.db.updatePatchHistory(this.cache.sourceVersion, this.cache.targetVersion).then(() => {
                            if (action === actions.UPDATE) {
                                ...
                                this.console.success(`DB rolled back to version ${this.cache.targetVersion}`);    
                            }
                        });
                    }
                });
