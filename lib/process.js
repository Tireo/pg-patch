...
const transactionMode = require('./common').transactionMode;
const dryRun = require('./common').dryRun;
    ...
    run: function() {
        ...
        return this.db.connect()
            ...
            .then(() => { //determine patch process information
                ...
                let shouldRejectVersion = function(versionType, invalidMsg, notFoundMsg){
                    if (cache[versionType] < 0) {
                        return Promise.reject(invalidMsg);
                    }
                    if (cache[versionType] > cache.maxPatchVersionFound) {
                        return Promise.reject(notFoundMsg);
                    }
                };

                let shouldReject = shouldRejectVersion(
                    'sourceVersion',
                    `Invalid sourceVersion: ${cache.sourceVersion}`,
                    `Source patch version not found: ${cache.sourceVersion}; (Max found was: ${cache.maxPatchVersionFound})`
                );
                if(shouldReject){
                    return shouldReject;
                }
                ...
                shouldReject = shouldRejectVersion(
                    'targetVersion',
                    `Invalid targetVersion: ${cache.targetVersion}`,
                    `Target patch version not found: ${config.targetVersion}; (Max found was: ${cache.maxPatchVersionFound})`
                );
                if(shouldReject){
                    return shouldReject;
                }
